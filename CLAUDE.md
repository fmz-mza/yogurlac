...
- La navbar (escritorio + menú móvil) la dibuja **`js/nav.js`**, una sola vez para todas las páginas (menos el login): cada HTML tiene `<script src="js/nav.js"></script>` donde va la barra, y el link activo se resalta solo según la URL. Para agregar o renombrar una pestaña se edita únicamente la lista `PAGINAS` de `nav.js`. Cada script de página sigue enganchando el toggle del menú móvil (`#mobile-menu-btn` / `#mobile-menu`); `nav.js` NO lo hace, para no alternarlo dos veces.
